...
We can generate tables for integer variables:
...
We need to transform our data to answer our initial question. Let's find the **daily** mean, max, and min temperatures in our `data.frame`. We can do this with the `summarize` function from the `dplyr` package. This package is part of the `tidyverse`, so the syntax is a bit different from what we've seen before.
...
The maximum **daily** temperature is `r met_daily$temp[nrow(met_daily)]` C at site `r met_daily$USAFID[nrow(met_daily)]` and the minimum daily temperature is `r met_daily$temp[1]`C at site `r met_daily$USAFID[1]`.
